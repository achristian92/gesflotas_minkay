...
begin
  ...
  -- kilometraje actual
  declare v_kilo_actual int;
-- kilometraje actual fecha
  declare v_fecha_kilo_actual date;
 -- kilometraje anterior
  declare v_kilo_anterior int;
 -- kilometraje anterior fecha
  declare v_fecha_kilo_anterior date;
 -- dias entre v_fecha_kilo_actual y v_fecha_kilo_anterior
  declare v_dias_generados int;
  -- intervalo de dias en mantenimiento
  set v_intervalo_dias = 7;
  -- consulto a la tabla tb_mantenimiento_vehiculos para obtener el ultimo mantenimineto por nro de placa
  select max(fecha_mantenimiento) into v_fecha_mantenimiento
  from tb_mantenimiento_vehiculos
  where nro_placa = v_nro_placa;
  -- obtener el kilometraje para obtener el kilometraje por dia (se asume que siempre va ser mayor)
  select kilometraje_recorrido,fecha_registro into v_kilo_actual,v_fecha_kilo_actual
  from   tb_historico_kilometraje
  where nro_placa = v_nro_placa  and fecha_registro = (select max(fecha_registro)
              from tb_historico_kilometraje
              where nro_placa = v_nro_placa);
 -- obtener el kilometraje  para obtener el kilometraje por dia (se asume que siempre va ser menor)
 select kilometraje_recorrido,fecha_registro into v_kilo_anterior,v_fecha_kilo_anterior
 from tb_historico_kilometraje
 where nro_placa = v_nro_placa and fecha_registro = (select min(xf.fecha_registro)
                        from  ( select fecha_registro
              from tb_historico_kilometraje
              where nro_placa = v_nro_placa
              order by fecha_registro desc
                limit 2) as xf ) ;
  
  select datediff(v_fecha_kilo_actual, v_fecha_kilo_anterior) into v_dias_generados;
  set v_kilometraje_recorrido = 1;
  set v_kilometraje_por_dias =  round(((v_kilo_actual-v_kilo_anterior) / v_dias_generados),2);
  set v_fecha_actual =  date_format(now(),"%y-%m-%d");
  
  -- regla para los de 5000 kilometros
  if (v_kilometraje_recorrido > 0 and v_kilometraje_recorrido <= 1000000 ) then
    set v_dias_transcurridos = round((5000/v_kilometraje_por_dias),0);
    set v_fecha_adelantada =  date_format(date_add(v_fecha_mantenimiento, interval v_dias_transcurridos day),"%y-%m-%d");
    set v_fecha_descontada =  date_format(date_sub(v_fecha_adelantada, interval v_intervalo_dias day),"%y-%m-%d");
    if (v_fecha_actual <= v_fecha_descontada ) then
       set v_respuesta = "SEGURO" ;
    elseif (v_fecha_actual > v_fecha_descontada and v_fecha_actual <=  v_fecha_adelantada) then
       set v_respuesta = "PROXIMO A VENCER" ;
    else
      if (v_fecha_actual > v_fecha_adelantada ) then
        set v_respuesta = "VENCIDO";
      end if;
    end if;
 
  else
   -- declarar mas intervalos para calcular el estado de mantenimiento
   set v_respuesta = "NINGUNO";
end if;
  return v_respuesta;
end $$
DELIMITER ;
...
DELIMITER $$
CREATE FUNCTION fdu_obt_kilo_actual(v_nro_placa CHAR(7)) RETURNS INT(11)
BEGIN DECLARE v_respuesta INT;
SELECT kilometraje_recorrido INTO v_respuesta
FROM tb_historico_kilometraje
WHERE nro_placa = v_nro_placa
  AND fecha_registro =
    (SELECT MAX(fecha_registro)
     FROM tb_historico_kilometraje
     WHERE nro_placa = v_nro_placa); RETURN v_respuesta; 
     END $$
DELIMITER ;

DELIMITER $$
CREATE FUNCTION fdu_obt_kilo_anterior( v_nro_placa CHAR(7)) RETURNS INT(11)
BEGIN
DECLARE v_respuesta INT;
  SELECT kilometraje_recorrido INTO v_respuesta
FROM tb_historico_kilometraje
WHERE nro_placa = v_nro_placa AND
            fecha_registro = (SELECT MIN(xf.fecha_registro)
            FROM  ( SELECT fecha_registro
            FROM tb_historico_kilometraje
            WHERE nro_placa = v_nro_placa
            ORDER BY fecha_registro DESC
...
DELIMITER $$
CREATE FUNCTION fdu_obt_total_kilometraje(v_nro_placa CHAR(7)) RETURNS INT(11)
BEGIN DECLARE v_respuesta INT;
SELECT SUM(kilometraje_recorrido) INTO v_respuesta
FROM tb_historico_kilometraje
WHERE nro_placa = v_nro_placa ; RETURN v_respuesta; 
END $$
DELIMITER ;
...
DELIMITER $$
CREATE FUNCTION func_fecha_ultimo_mant(v_nro_placa CHAR(7)) RETURNS varchar(200) CHARSET latin1
BEGIN
 declare v_respuesta varchar(200);
 -- obtener fecha del ultimo mantenimiento
  SELECT MAX(fecha_mantenimiento) INTO v_respuesta
  FROM tb_mantenimiento_vehiculos
  WHERE nro_placa = v_nro_placa;
...
DELIMITER $$
CREATE FUNCTION func_km_x_dia(v_nro_placa CHAR(7)) RETURNS varchar(200) 
BEGIN
 declare v_respuesta varchar(200);
   -- kilometraje actual
  DECLARE v_kilo_actual INT;
-- kilometraje actual fecha
 ...
 -- kilometraje anterior fecha
  ...
  -- obtener ultimo el kilometraje y fecha  para obtener el kilometraje por dia (se asume que siempre va ser mayor)
  select kilometraje_recorrido,fecha_registro into v_kilo_actual,v_fecha_kilo_actual
  from   tb_historico_kilometraje
  where nro_placa = v_nro_placa  and fecha_registro = (select max(fecha_registro)
              from tb_historico_kilometraje
  ...
  -- obtener penultimo el kilometraje y fecha  para obtener el kilometraje por dia (se asume que siempre va ser menor)
 SELECT kilometraje_recorrido,fecha_registro INTO v_kilo_anterior,v_fecha_kilo_anterior
 FROM tb_historico_kilometraje
 WHERE nro_placa = v_nro_placa AND fecha_registro = (SELECT MIN(xf.fecha_registro)
                        FROM  ( SELECT fecha_registro
  ...
  -- generamos cantidad de dias transcurridos               
 SELECT DATEDIFF(v_fecha_kilo_actual, v_fecha_kilo_anterior) INTO v_dias_generados;  
 
 SET v_respuesta =  ROUND(((v_kilo_actual-v_kilo_anterior) / v_dias_generados),2);           
...
DELIMITER $$
CREATE FUNCTION func_prox_fecha_mant(v_nro_placa CHAR(7)) RETURNS varchar(200) 
BEGIN
 DECLARE v_respuesta date;
   -- kilometraje actual
  DECLARE v_kilo_actual INT;
-- kilometraje actual fecha
  DECLARE v_fecha_kilo_actual DATE;
 ...
 -- kilometraje anterior fecha
  DECLARE v_fecha_kilo_anterior DATE;
  DECLARE v_dias_generados INT;
  -- lo ultimo que se agrego desde km_x_dias
    -- dias transcurridos
  declare v_dias_transcurridos int;
    -- se obtendra la fecha maxima de un mantenimiento
  DECLARE v_fecha_mantenimiento DATE;
 ...
 SET v_kilometraje_por_dias =  ROUND(((v_kilo_actual-v_kilo_anterior) / v_dias_generados),2);           
  ...
  -- demas es una copia para sacar km_x_dia (consultamos otra tabla)
  ...
  WHERE nro_placa = v_nro_placa;
   ...
   set v_dias_transcurridos = round((5000/v_kilometraje_por_dias),0);
   set v_respuesta =  date_format(date_add(v_fecha_mantenimiento, interval v_dias_transcurridos day),"%y-%m-%d");
...
DELIMITER $$
CREATE FUNCTION func_kilo_ultimo_mant(v_nro_placa CHAR(7)) RETURNS INT(11)
BEGIN DECLARE v_respuesta INT;
SELECT kilometraje_recorrido INTO v_respuesta
FROM tb_historico_kilometraje
WHERE nro_placa = v_nro_placa
...
DELIMITER $$
CREATE FUNCTION fdu_estado_soat(v_nro_placa CHAR(7)) RETURNS VARCHAR(200) CHARSET latin1
BEGIN
 DECLARE v_respuesta VARCHAR(200);
 DECLARE v_fecha_vencimiento_soat DATE;
 DECLARE v_intervalo_dias INT;
 DECLARE v_fecha_actual DATE;
 DECLARE v_fecha_descontada DATE;
 ...
 SET v_intervalo_dias = 7;
  ...
  SELECT fecha_vencimiento_soat INTO v_fecha_vencimiento_soat
  FROM tb_soat_vehiculo
  WHERE nro_placa = v_nro_placa AND estado_soat ='1' ;
  
  SET v_fecha_descontada =  DATE_FORMAT(DATE_SUB(v_fecha_vencimiento_soat, INTERVAL v_intervalo_dias DAY),"%y-%m-%d");
  
  
  IF(v_fecha_actual > v_fecha_vencimiento_soat) THEN 
  SET v_respuesta = "S_VENCIDO";
  ELSEIF (v_fecha_actual > v_fecha_descontada AND v_fecha_actual <=  v_fecha_vencimiento_soat) THEN
  SET v_respuesta = "S_PROXIMO_A_VENCER";
  ELSEIF(v_fecha_actual < v_fecha_descontada) THEN
  SET v_respuesta = "S_SEGURO";
  else
  set v_respuesta = "SIN DATOS";
  END IF;
